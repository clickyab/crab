...
func Access(next framework.Handler) framework.Handler {
	return func(ctx context.Context, w http.ResponseWriter, r *http.Request) {
		// check if d is valid
		d, err := models.NewModelsManager().FindActiveDomainByName(r.Host)
		if err != nil {
...
func MustGetDomain(ctx context.Context) *models.Domain {
	rd, ok := GetDomain(ctx)
...
func GetDomain(ctx context.Context) (*models.Domain, bool) {
	rd, ok := ctx.Value(ContextDomain).(*models.Domain)
	if !ok {
		return nil, false
	}
