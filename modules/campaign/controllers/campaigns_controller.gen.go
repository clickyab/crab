...
func (u *Controller) listCampaigns(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	dt, cnt, err := m.FillCampaigns(pc, filter, from, to, search, params, queryParams, sort, order, p, c)
	if err != nil {
		u.JSON(w, http.StatusBadRequest, err)
		return
	}
...
func (u *Controller) defCampaigns(ctx context.Context, w http.ResponseWriter, r *http.Request) {
	...
	u.OKResponse(
		w,
		listCampaignsDefResponse{Checkable: false, SearchKey: "q", QueryParams: "", Multiselect: false, DateFilter: "", Hash: hash, Columns: listCampaignsDefinition},
	)
}
